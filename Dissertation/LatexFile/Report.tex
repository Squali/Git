...
\begin{document}	
	...
	\begin{enumerate}
		...
		\item $\sigma \sim InvGamma(0.01,0.01)$, $\rho \sim \mathcal{U}(0,1)$
	...
	\end{equation}
	\end{enumerate}
	\item Choose a proposal distribution $q(\theta^{*}, (\lambda)^{*} |\theta, (\lambda))$. It needs to be easy to simulate and determine an irreducible chain. We can take a log-normal distribution for each element of $log(\lambda)^{*}$ with median $log(\lambda_{i,j})$. A normal distribution for \boldmath $\beta, a, b$ \unboldmath, and finally a normal distribution for $\sigma$ and $\mathcal{U}[0, 1]$ for $\rho$.
